...
# the project does and who it is for. It is shown instead of the GitHub
# description. Repositories without an entry fall back to a summary derived from
...
def build_repo_lines(repositories: list[dict[str, object]]) -> str:
    ...
    lines = []
    for position, (name, html_url, summary, badges) in enumerate(entries, start=1):
        marker = f"{position}. "
        indent = " " * len(marker)
        if name.casefold() not in REPO_SUMMARIES:
            summary = markdown_pattern.sub(r"\\\1", summary)
        lines.append(f"{marker}[{name}]({html_url}) — {summary} \n{indent}{badges}")
